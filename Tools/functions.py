...
class TestFunctions():
    ...
    def check_grassninja_port(self, *args):
        self.log.item_start("Check Grassninja Port")
        result = False, "--FAIL--"
        try:
            p = glob.glob("/dev/cu.*")
            for i in range(len(p)):
                for x in range(len(p) - i - 1):
                    if p[x][:-2] == p[x + 1][:-2]:
                        result = True, "--PASS--"
                        break
                if result[0]:
            ...
            time.sleep(0.5)
            if result[0]:
                self.log.set_item_result("serial port normal,serial port list:{}".format(str(p)), "PASS")
            else:
                self.log.set_item_result("Not found serial port!serial port list:{}".format(str(p)), "FAIL")
        except Exception as e:
            self.log.set_item_error(str(e))
        self.log.item_end("Check Grassninja Port")
        return result

    def fw_update(self, *args):
        self.log.item_start("FW Update")
        result = False, "--FAIL--"
        output = ''
        try:
            cmd = "/bin/sh /Users/gdlocal/Documents/DFU_FWDL_Folders00/goldrestore_fixture.sh"
            output += "Run Cmd : {}\n".format(cmd)
            for _ in range(2):
                return_code, stdout, stderr = RunShellWithTimeout(cmd, 200)
                output += "return_code:{}\n".format(return_code)
                ...
                if return_code != 0:
                    ...
                    continue
                if 'SN1234567: success' in stdout.decode():
                    result = True, "--PASS--"
                    break
        except Exception as e:
            self.log.set_item_error(str(e))
        self.log.set_item_result(output, "--PASS--" if result[0] else "--FAIL--")
        self.log.item_end("FW Update")
        return result
